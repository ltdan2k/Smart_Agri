...
function drawChartDoAm() {
  ...
  buttonRef1.on("value", function(snapshot) {
    ...
    // Lấy trạng thái ON/OFF của button từ Firebase
    var buttonRef2 = firebase.database().ref('DK').child('auto');
  ...
  function WriteDataTofb(lamp,fan,pump,auto,manual)
{
...
function clicklamp(){
  ...
  WriteDataTofb(valueLamp,valueFan,valuePump,valueAuto,valueManual);
}
